...
public class PhoneARCamera : MonoBehaviour
{
    ...
    {
        const int maxDetections = 2; // Maximum number of object detections to keep
        ...
        // Limit the number of object detections
        if (this.boxSavedOutlines.Count > maxDetections)
        {
            this.boxSavedOutlines.RemoveRange(0, boxSavedOutlines.Count - maxDetections);
        }
    ...
    private void DrawBoxOutline(BoundingBox outline, float scaleFactor, float shiftX, float shiftY)
    {
        ...
        DrawLabel(new Rect(x, y - 80, 200, 20), $"Mendeteksi benda...");
    }
