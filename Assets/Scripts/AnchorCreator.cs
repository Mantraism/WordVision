    ...
    void Update()
    {
        ...
        if (anchorDic.Count != 0)
        {
            ...
            foreach (KeyValuePair<ARAnchor, BoundingBox> pair in anchorDic)
            {
                ...
                {
                    Debug.Log($"DEBUG: anchor removed. {pair.Value.Label}");
        ...
        int createdAnchors = 0; // Counter for the number of created anchors
        ...
        foreach (var outline in boxSavedOutlines)
        {
            ...
            if (Pos2Anchor(center_x, center_y, outline))
            {
                ...
                outline.Used = true;
                createdAnchors++; // Increment the counter
                // Check if the maximum number of anchors has been reached
                if (createdAnchors >= maxAnchors)
                {
                    s_Hits.Clear();
                    break; // Break the loop if the maximum number of anchors is reached
                }
            }
            else
            {
